...
?>
<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#07111f"><title>Create Trainer · Pokémon Vortex</title><link rel="stylesheet" href="<?= pv_h(pv_asset('css/vortex-modern.css')) ?>?v=<?= pv_asset_version() ?>"><script defer src="<?= pv_h(pv_asset('js/vortex-modern.js')) ?>?v=<?= pv_asset_version() ?>"></script>
<style>.pv-starters{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:7px;margin:8px 0 16px}.pv-starter{position:relative}.pv-starter input{position:absolute;opacity:0}.pv-starter label{display:block;padding:8px 4px;text-align:center;border:1px solid rgba(90,180,239,.16);border-radius:10px;background:rgba(3,12,23,.42);cursor:pointer}.pv-starter label img{width:58px;height:58px;object-fit:contain;display:block;margin:auto}.pv-starter label span{display:block;font-size:10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.pv-starter input:checked+label{border-color:#58d8ff;background:rgba(42,139,220,.16);box-shadow:0 0 0 2px rgba(72,191,255,.08)}@media(max-width:700px){.pv-starters{grid-template-columns:repeat(4,1fr)}}</style></head>
...
<section class="pv-auth-side"><a class="pv-brand" href="<?= pv_h(pv_url('index.php')) ?>"><span class="pv-brand-mark">PV</span><span class="pv-brand-copy">POKÉMON VORTEX<small>NEW TRAINER</small></span></a><h1 style="margin-top:38px">Begin your<br><span style="color:#76ddff">adventure.</span></h1><p class="pv-subtle">Choose your trainer identity and starter. Your first Pokémon begins at level 18 with randomized IVs, nature, gender and an available ability.</p><img src="<?= pv_h(pv_static('images/frontv3.png')) ?>" alt="Battle Arena"></section>
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px"><div class="pv-field"><label>Username</label><input name="username" maxlength="24" value="<?= pv_h($_POST['username']??'') ?>" autocomplete="username" required></div><div class="pv-field"><label>Email</label><input name="email" type="email" maxlength="190" value="<?= pv_h($_POST['email']??'') ?>" autocomplete="email" required></div></div>
<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px"><div class="pv-field"><label>Password</label><input name="password" type="password" minlength="8" autocomplete="new-password" required></div><div class="pv-field"><label>Confirm password</label><input name="password2" type="password" minlength="8" autocomplete="new-password" required></div></div>
...
<label style="display:block;margin:3px 0 7px;font-size:12px;font-weight:800">Choose your starter Pokémon</label><div class="pv-starters">
