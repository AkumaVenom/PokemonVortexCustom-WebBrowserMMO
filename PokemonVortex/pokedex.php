...
<?php if ($detailRequested): ?>
    ...
    <div class="pv-dex-detail-grid pv-dex-detail-grid-production">
        ...
        <section class="pv-card pv-dex-specimen pv-dex-selected-specimen">
            ...
            <?php if (trim((string)($owned['nickname']??'')) !== ''): ?><p class="pv-subtle">Nickname: <strong><?=pv_h((string)$owned['nickname'])?></strong></p><?php endif; ?>
            <p class="pv-subtle">Effort values (HP / ATK / DEF / SP.ATK / SP.DEF / SPEED): <strong><?=implode(' / ',array_map(static fn($field)=>(string)(int)($owned[$field.'_ev']??0),['hp','attack','defense','spatk','spdef','speed']))?></strong></p>
