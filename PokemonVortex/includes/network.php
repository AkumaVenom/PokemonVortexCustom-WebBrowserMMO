...
function pv_network_friend_request(mysqli $db,int $uid,int $target): void
{
    ...
    if(!$stmt)throw new RuntimeException('The friend request could not be created.');$stmt->bind_param('iii',$uid,$target,$now);if(!$stmt->execute()){ $stmt->close();throw new RuntimeException('The friend request could not be created.');}$stmt->close();
}
...
{
    $stmt=$db->prepare('DELETE FROM trainer_friends WHERE (user_id=? AND friend_id=?) OR (user_id=? AND friend_id=?)');if(!$stmt)throw new RuntimeException('The friendship could not be updated.');$stmt->bind_param('iiii',$uid,$friendId,$friendId,$uid);$stmt->execute();$stmt->close();
}
...
{
    $stmt=$db->prepare('DELETE FROM trainer_blocks WHERE user_id=? AND blocked_user_id=?');if(!$stmt)throw new RuntimeException('The block could not be removed.');$stmt->bind_param('ii',$uid,$target);$stmt->execute();$stmt->close();
}
...
function pv_network_send_message(mysqli $db,int $uid,int $receiverId,string $subject,string $body): int
{
    ...
    $now=time();$stmt=$db->prepare('INSERT INTO trainer_messages (sender_id,receiver_id,subject,body,created_at,read_at,sender_deleted,receiver_deleted) VALUES (?,?,?,?,?,0,0,0)');if(!$stmt)throw new RuntimeException('The message could not be sent.');$stmt->bind_param('iissi',$uid,$receiverId,$subject,$body,$now);if(!$stmt->execute()){ $stmt->close();throw new RuntimeException('The message could not be sent.');}$id=(int)$db->insert_id;$stmt->close();return $id;
}
...
{
    $stmt=$db->prepare('UPDATE trainer_messages SET receiver_deleted=IF(receiver_id=?,1,receiver_deleted),sender_deleted=IF(sender_id=?,1,sender_deleted) WHERE id=? AND (receiver_id=? OR sender_id=?)');if(!$stmt)throw new RuntimeException('The message could not be removed.');$stmt->bind_param('iiiii',$uid,$uid,$messageId,$uid,$uid);$stmt->execute();$stmt->close();
}
...
function pv_clan_apply(mysqli $db,int $uid,int $clanId): void
{
    ...
    $stmt=$db->prepare("INSERT INTO clan_applications (clan_id,user_id,status,created_at,resolved_at) VALUES (?,?,'pending',?,0)");if(!$stmt)throw new RuntimeException('The clan application could not be submitted.');$stmt->bind_param('iii',$clanId,$uid,$now);$stmt->execute();$stmt->close();
}
...
function pv_clan_resolve_application(mysqli $db,int $leaderId,int $applicationId,string $decision): void
{
    ...
    try{
        ...
        $db->commit();pv_recalculate_clan_progress($db,(string)$clan['name']);
...
function pv_clan_leave(mysqli $db,int $uid): void
{
    ...
    $db->begin_transaction();try{$stmt=$db->prepare('DELETE FROM clan_members WHERE id=?');if(!$stmt)throw new RuntimeException('Clan membership could not be removed.');$stmt->bind_param('i',$uid);$stmt->execute();$stmt->close();$stmt=$db->prepare("UPDATE members SET clan_name='',clan_tag='' WHERE id=?");if($stmt){$stmt->bind_param('i',$uid);$stmt->execute();$stmt->close();}$db->commit();pv_recalculate_clan_progress($db,$name);}catch(Throwable $e){$db->rollback();if($e instanceof RuntimeException)throw $e;throw new RuntimeException('You could not leave the clan. Please try again.');}
}
...
function pv_clan_remove_member(mysqli $db,int $leaderId,int $memberId): void
{
    ...
    $db->begin_transaction();try{$stmt=$db->prepare('DELETE FROM clan_members WHERE id=? AND clan_id=? AND owner=0');if(!$stmt)throw new RuntimeException('The clan member could not be removed.');$stmt->bind_param('ii',$memberId,$clanId);$stmt->execute();$ok=$stmt->affected_rows===1;$stmt->close();if(!$ok)throw new RuntimeException('That trainer is no longer a removable member of this clan.');$stmt=$db->prepare("UPDATE members SET clan_name='',clan_tag='' WHERE id=?");if($stmt){$stmt->bind_param('i',$memberId);$stmt->execute();$stmt->close();}$db->commit();pv_recalculate_clan_progress($db,$name);}catch(Throwable $e){$db->rollback();if($e instanceof RuntimeException)throw $e;throw new RuntimeException('That member could not be removed. Please try again.');}
}
...
function pv_clan_disband(mysqli $db,int $leaderId): void
{
    ...
    $db->begin_transaction();try{$stmt=$db->prepare('SELECT id FROM clan_members WHERE clan_id=? FOR UPDATE');if(!$stmt)throw new RuntimeException('Clan membership could not be locked.');$stmt->bind_param('i',$clanId);$stmt->execute();$r=$stmt->get_result();$ids=[];while($row=$r->fetch_assoc())$ids[]=(int)$row['id'];$stmt->close();foreach($ids as $id){$stmt=$db->prepare("UPDATE members SET clan_name='',clan_tag='' WHERE id=?");if($stmt){$stmt->bind_param('i',$id);$stmt->execute();$stmt->close();}}$stmt=$db->prepare('DELETE FROM clan_applications WHERE clan_id=?');if($stmt){$stmt->bind_param('i',$clanId);$stmt->execute();$stmt->close();}$stmt=$db->prepare('DELETE FROM clan_members WHERE clan_id=?');if($stmt){$stmt->bind_param('i',$clanId);$stmt->execute();$stmt->close();}$stmt=$db->prepare('DELETE FROM clans WHERE id=?');if(!$stmt)throw new RuntimeException('The clan could not be disbanded.');$stmt->bind_param('i',$clanId);$stmt->execute();$stmt->close();$db->commit();}catch(Throwable $e){$db->rollback();if($e instanceof RuntimeException)throw $e;throw new RuntimeException('The clan could not be disbanded. Please try again.');}
}
