...
<section class="nxt-trainer-hero">
  <div class="nxt-trainer-hero-copy"><span class="pv-eyebrow">TRAINER HOME</span><h1>Welcome back, <?=pv_h((string)$member['username'])?>.</h1><p>Your next adventure is waiting.</p>
  <div class="pv-actions"><a class="pv-button" href="<?=pv_h(pv_url('map_select.php'))?>">Explore the world</a><a class="pv-button pv-button-secondary" href="<?=pv_h(pv_url('rival_hub.php'))?>">Challenge a rival</a></div></div>
  <div class="nxt-trainer-partners" aria-hidden="true">
    <img class="nxt-partner-lead" src="<?=pv_h(pv_pokemon_image((string)($team[0]['name']??'Pikachu')))?>" alt="" width="112" height="112" loading="eager">
    <?php foreach(['eevee'=>'Eevee','charizard'=>'Charizard','lucario'=>'Lucario','gengar'=>'Gengar'] as $artKey=>$artName): ?><img class="nxt-partner-<?=pv_h($artKey)?>" src="<?=pv_h(pv_pokemon_image($artName))?>" alt="" width="112" height="112" loading="eager"><?php endforeach; ?>
  </div>
</section>

<section class="pv-grid cols-4 nxt-trainer-stats" style="margin-top:0"><article class="pv-card"><div class="pv-kpi"><?= number_format($money) ?></div><div class="pv-kpi-label">PokéMoney</div></article><article class="pv-card"><div class="pv-kpi"><?= number_format($collection) ?></div><div class="pv-kpi-label">Pokémon owned</div></article><article class="pv-card"><div class="pv-kpi"><?= number_format($unique) ?></div><div class="pv-kpi-label">Unique forms</div></article><article class="pv-card"><div class="pv-kpi"><?= number_format($battles) ?></div><div class="pv-kpi-label">Battles completed</div></article></section>

<section class="pv-card nxt-team-panel" style="margin-top:12px"><div class="pv-page-head"><div><h3 style="margin:0">Active team</h3><div class="pv-subtle">Your six battle slots</div></div><a href="<?= pv_h(pv_url('change_team.php')) ?>">Change team →</a></div>
<div class="pv-team">
